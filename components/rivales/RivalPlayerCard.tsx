...
export function RivalPlayerCard({
  ...
  return (
    <Card
      className="overflow-hidden relative group bg-slate-900/60 border-slate-800/80 hover:border-slate-700 hover:shadow-xl hover:shadow-black/40 transition-all duration-300 flex flex-col"
    >
      {/* Franja superior decorativa según posición */}
      <div className={`absolute top-0 left-0 right-0 h-1.5 ${stripeColor}`} />

      {/* Zona interactiva accesible de la tarjeta */}
      <div
        role="button"
        tabIndex={0}
        aria-label={`Ver ficha detallada de ${player.nombre}`}
        onClick={() => onClick?.(player)}
        onKeyDown={handleKeyDown}
        className="flex-1 flex flex-col cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#CC0E21] focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 rounded-t-2xl transition-all duration-200"
      >
      ...
      {/* Acciones para Modo Edición (fuera del botón para no anidar elementos interactivos) */}
      {isEditMode && (
        <div
          onClick={(e) => e.stopPropagation()}
          className="flex items-center justify-center gap-2 w-full px-4 pb-3 pt-2 border-t border-slate-800/70 z-10"
        >
          <Button
            variant="ghost"
