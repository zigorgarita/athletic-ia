...
import { CheckCircle2, XCircle, AlertCircle, ShieldAlert, UserCheck } from 'lucide-react';
...
export function ReviewExtractedReportModal({
  ...
  // Unificar observaciones en un estado editable local
  const [observations, setObservations] = useState<Observation[]>(() => {
    ...
    if (extraction.observacionesRival) {
      Object.entries(extraction.observacionesRival).forEach(([cat, list]) => {
        if (Array.isArray(list)) {
          list.forEach(obs => {
            result.push({
              ...obs,
              categoria: obs.categoria || cat,
              estado: obs.estado || 'aprobado', // Por defecto marcadas para fácil aprobación
    ...
    if (extraction.propuestasDelAnalista) {
      Object.entries(extraction.propuestasDelAnalista).forEach(([cat, list]) => {
        if (Array.isArray(list)) {
          list.forEach(obs => {
            result.push({
              ...obs,
              categoria: obs.categoria || cat,
              estado: obs.estado || 'aprobado',
              confianza: obs.confianza || 'alta',
              prioridad: obs.prioridad || 'normal',
  ...
  // Estado editable para amenazas de jugadores rivales
  const [threats, setThreats] = useState<RivalPlayerThreat[]>(() => {
    return extraction?.amenazasJugadores || [];
  });
  ...
  const handleConfirmAndIntegrate = async () => {
    try {
      ...
      if (approvedObs.length === 0 && threats.length === 0) {
        setFeedbackMsg({ type: 'error', text: 'Debes aprobar al menos una observación para integrar en la ficha.' });
        setIsSaving(false);
        return;
      }
      ...
      // También guardar las amenazas de jugadores en formato observación si existen
      threats.forEach(t => {
        if (t.dorsal || t.nombre) {
          payloadRows.push({
            ...
            content: `[Dorsal ${t.dorsal || 'S/N'}] ${t.nombre || 'Jugador Rival'} (${t.posicionHabitual || 'Posición'}): ${t.observaciones}. Fortalezas: ${t.fortalezas.join(', ')}. ${t.consignaEspecifica || ''}`,
            source_type: 'texto',
            page: 1,
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-100 flex items-center gap-2">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-800 text-emerald-400 font-semibold">
                  IA + Validación Entrenador
                </span>
              </h2>
        ...
        <div className="px-6 py-3 bg-slate-950/50 border-b border-slate-800/80 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400">
              Aprobadas: <strong className="text-emerald-400">{totalAprobadas}</strong> / {observations.length}
            </span>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          {activeTab !== 'players' ? (
                ...
                >
                  <div className="flex justify-between items-start gap-4 mb-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] text-slate-500">Pág. {obs.pagina || 1} ({obs.fuente})</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-all ${
                          obs.estado === 'aprobado' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-800 text-slate-400 hover:text-emerald-400'
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-all ${
                          obs.estado === 'rechazado' ? 'bg-red-600 text-white shadow-sm' : 'bg-slate-800 text-slate-400 hover:text-red-400'
                        }`}
                      >
                  ...
                  <textarea
                    ...
                    disabled={!isEditMode}
                    rows={2}
                    className="w-full bg-slate-950/60 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-[#CC0E21]/50 transition-all resize-y"
                  />

                  {/* Ajustes de Confianza y Prioridad */}
                  <div className="mt-3 pt-3 border-t border-slate-800/60 flex justify-between items-center text-xs text-slate-400 flex-wrap gap-2">
            ...
            /* Sección de Amenazas de Jugadores Rival */
            <div className="space-y-4">
              <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 text-xs text-slate-300">
                <h4 className="font-bold text-slate-100 flex items-center gap-2 mb-1">
                  ...
                  Amenazas de Jugadores Rivales Detectadas
                </h4>
                <p className="text-slate-400 text-[11px]">
                  Estas alertas se relacionarán de forma automática con la alineación y posiciones afectadas en la Pizarra Táctica (ej. Extremo Derecho rival ➔ Lateral Izquierdo propio).
                </p>
              </div>
              ...
              {threats.map((t, idx) => (
                <div key={idx} className="bg-slate-900 border border-amber-500/30 rounded-2xl p-4 space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full bg-red-950 border border-red-800 text-red-400 text-[10px] font-bold uppercase">
                      Peligro {t.nivelPeligro}
                    </span>
                  </div>

                  <p className="text-xs text-slate-300">{t.observaciones}</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs bg-slate-950/50 p-3 rounded-xl border border-slate-800">
                    <div>
                      <strong className="text-slate-400 text-[10px] uppercase block mb-1">Fortalezas:</strong>
                      <div className="flex flex-wrap gap-1">
                        {t.fortalezas.map((f, i) => (
                    ...
                    <div>
                      ...
                      <span className="text-emerald-400 font-medium">{t.nuestroPuestoAfectadoDirecto || 'Lateral Izquierdo'}</span>
                    </div>
                  </div>

                  <div>
                    <strong className="text-slate-400 text-[10px] uppercase block mb-1">Consigna Recomendada para la Pizarra:</strong>
                    <input
                      type="text"
                      value={t.consignaEspecifica || ''}
                      onChange={e => {
                        const newThreats = [...threats];
                        newThreats[idx].consignaEspecifica = e.target.value;
                        setThreats(newThreats);
                      }}
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-[#CC0E21]"
                    />
                  </div>
                </div>
              ))}
