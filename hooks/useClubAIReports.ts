...
export function useClubAIReports(seasonId: string | undefined) {
  ...
  const saveReport = async (data: Partial<ClubAIReport>): Promise<boolean> => {
    try {
      ...
      const res = await fetch('/api/clubs/ai-reports', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || `Error al guardar informe de IA (${res.status})`);
      }
  ...
  const deleteReport = async (id: string): Promise<boolean> => {
    try {
      verifyWritePermission();
      const res = await fetch(`/api/clubs/ai-reports?id=${encodeURIComponent(id)}`, {
        method: 'DELETE',
      });

      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.error || `Error al borrar informe de IA (${res.status})`);
      }
