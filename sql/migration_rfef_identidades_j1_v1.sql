-- ============================================================================
-- RFEF · J1-E · IMPORTACIÓN DEFENSIVA DE IDENTIDADES DE JUGADORES (JORNADA 1)
-- 
-- OBJETIVO:
-- 1. Asignar exclusivamente rfef_player_id a los 15 jugadores documentales de DEPORTIVO ALAVÉS.
-- 2. Insertar 218 nuevos jugadores con origen='rfef' para los 13 rivales de J1.
-- 3. Cero jugadores propios de SD INDAUTXU en club_players (Análisis Propio intacto).
-- 4. Preservar intactos nombres, fotos, posiciones y dorsales documentales de Alavés.
-- 5. No convertir dorsal_partido en dorsal permanente (dorsal = NULL para los nuevos).
--
-- CARACTERÍSTICAS:
-- - Transaccional y atómico (DO block en PostgreSQL).
-- - Prechecks exhaustivos antes de cualquier modificación.
-- - Postchecks obligatorios al finalizar la transacción.
-- - Rollback automático si falla cualquier verificación.
--
...
BEGIN
    RAISE NOTICE '============================================================';
    RAISE NOTICE '   RFEF · J1-E · INICIANDO TRANSACCIÓN DE IDENTIDADES J1    ';
    RAISE NOTICE '============================================================';
    ...
    -- C. Verificar que no existe ningún registro de SD Indautxu en club_players
    SELECT COUNT(*) INTO v_indautxu_check
    FROM public.club_players
    WHERE UPPER(nombre) LIKE '%INDAUTXU%';

    IF v_indautxu_check > 0 THEN
        RAISE EXCEPTION '[PRECHECK FALLIDO] Ya existen registros anómalos de Indautxu en club_players';
    END IF;
    ...
    -- ------------------------------------------------------------------------
    RAISE NOTICE 'Actualizando rfef_player_id en los 15 futbolistas documentales de Alavés...';

    UPDATE public.club_players
    SET rfef_player_id = 1527285
    WHERE id = '825c5e3a-4360-4a70-8d9b-a749725620d7'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1539163
    WHERE id = '0d3e16d1-b6d2-4b90-89dc-7779b09d69f8'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 900379315
    WHERE id = 'dfb54a40-fcac-496c-acd2-0c75ac769fca'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1515418
    WHERE id = 'ae3da1c6-be38-4bb4-a9ba-db85614074b1'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1489183
    WHERE id = 'faa630cb-b908-406d-bf01-31edcdfbfcaf'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1288502
    WHERE id = '5553e4a1-6943-4083-9c6f-f45cbf62cd74'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1492952
    WHERE id = '37772ba7-7f67-4594-ada0-4ff121789454'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1527283
    WHERE id = 'a71a74ff-4dae-45cc-88e1-85b9a65536f1'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 900752896
    WHERE id = 'af567358-d3b7-468c-ba41-3f3de606d136'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 655417
    WHERE id = '6b6d269c-fe49-480b-99b9-69cde109584a'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1176078
    WHERE id = '6b34cf42-0551-4a03-8631-9f1ee1141989'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1287698
    WHERE id = 'd91e478f-2470-4aaf-9835-5e48edb3c3b0'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1512654
    WHERE id = '7e930c32-1d0a-47e1-a820-8a66930bdaba'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 1220291
    WHERE id = 'a7a1fb12-51c4-4225-9df2-458ccd8f4ee0'::uuid AND origen = 'documento';
    UPDATE public.club_players
    SET rfef_player_id = 900656259
    WHERE id = 'c57793a5-5f5c-40fd-9c6a-16f449a5d10d'::uuid AND origen = 'documento';
    ...
    -- A. Exactamente 218 nuevos registros con origen='rfef' creados
    ...
    IF v_total_insertados <> 218 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Nuevos jugadores con origen=rfef = % (esperado: 218)', v_total_insertados;
    END IF;
    ...
    -- F. SD Indautxu continúa con 0 club_players
    SELECT COUNT(*) INTO v_indautxu_check
    FROM public.club_players
    WHERE UPPER(nombre) LIKE '%INDAUTXU%';

    IF v_indautxu_check > 0 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Se detectaron registros de Indautxu en club_players';
    END IF;
    ...
    -- H. Distribución exacta de las 218 altas por club_season_id según J1-D
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '59d14dd9-b28b-4f8e-9889-c56f6a2eaeca'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en SD LEIOA (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '48bafceb-6b77-478d-86d1-5af6ed3d23a2'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en CULTURAL LEONESA (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '5545c49b-0756-4408-9412-e19ce28b75d1'::uuid AND origen = 'rfef') <> 3 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en DEPORTIVO ALAVÉS (esperado: 3 altas)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '32f8b024-b52b-4d49-8cc9-4827cdd6aec6'::uuid AND origen = 'rfef') <> 17 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en ARRATIA (esperado: 17)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = 'eea969ce-45de-43ba-b013-c5bf379be93d'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en SANTUTXU FC (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = 'c6e8b8eb-af23-48d3-b1bb-052fffb54815'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en DANOK BAT (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '1fdff3a9-21d1-4f0b-84ec-007e125bb8b3'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en EF MAREO (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '6383ab39-bae4-4668-80d0-2f8110fa3610'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en REAL VALLADOLID (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '09ae366d-0822-4344-9b31-edfa833fc776'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en BETOÑO (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '6d3543e6-2f2c-4d77-8213-3718ff785e53'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en SD EIBAR (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '65e71700-f8ee-4d02-bb0a-05d87bed6a35'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en UNIONISTAS SALAMANCA (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '692b8b68-aa45-4e5e-9547-845eaef03452'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en ANTIGUOKO KE (esperado: 18)';
    END IF;
    IF (SELECT COUNT(*) FROM public.club_players WHERE club_season_id = '9e01cbcd-e3f5-4208-bd84-4fda4e34a068'::uuid AND origen = 'rfef') <> 18 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Distribución errónea en REAL SOCIEDAD (esperado: 18)';
    END IF;
    ...
    RAISE NOTICE '============================================================';
    RAISE NOTICE '   RFEF · J1-E · TRANSACCIÓN VALIDADA EXITOSAMENTE (PASS)   ';
    ...
    RAISE NOTICE '   - SD Indautxu y Análisis Propio 100%% protegidos         ';
