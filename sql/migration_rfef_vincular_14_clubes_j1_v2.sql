-- ============================================================================
-- RFEF · J1-C.1 · VINCULACIÓN DEFENSIVA DE LOS 14 CLUBES FEDERATIVOS (V2)
-- 
-- OBJETIVO:
-- 1. Crear identidad federativa técnica de SD INDAUTXU en clubs:
--    - id generado automáticamente por PostgreSQL (gen_random_uuid()).
--    - nombre = 'SD INDAUTXU'.
--    - tipo = 'PROPIO'.
--    - rfef_club_id = 33836524.
--    - escudo_url = '/escudo.jpg'.
-- 2. Vincular exclusivamente rfef_club_id a los 13 rivales existentes de División de Honor Juvenil G2.
-- 3. Comprobar explícitamente en el postcheck los 13 pares exactos: UUID <-> rfef_club_id.
-- 4. Respetar estrictamente a ATHLETIC CLUB (205514) y UD LOGROÑES (205744) ya vinculados.
-- 5. NO crear club_seasons para Indautxu.
-- 6. NO crear jugadores en club_players.
-- 7. ANÁLISIS PROPIO: NI TOCAR (players, matches, evaluaciones, etc. 100% aislados).
--
-- CARACTERÍSTICAS:
-- - Transaccional y atómico (bloque DO en PostgreSQL).
-- - Prechecks exhaustivos (valida UUIDs, estados previos, colisiones de IDs).
-- - Postchecks endurecidos (16 clubes, 15 LIGA, 1 PROPIO, 13 pares exactos validados, Indautxu localizado por rfef_club_id).
...
DECLARE
    ...
    v_verificados_13 INTEGER;
    v_indautxu_id UUID;
    v_indautxu_post_tipo TEXT;
    v_indautxu_post_nombre TEXT;
BEGIN
    RAISE NOTICE '============================================================';
    RAISE NOTICE '   RFEF · J1-C.1 · TRANSACCIÓN DEFENSIVA DE CLUBES (V2)     ';
    RAISE NOTICE '============================================================';
    ...
    --    (Omitiendo id para que PostgreSQL genere automáticamente el UUID)
    -- ------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------
    -- 4. POSTCHECKS OBLIGATORIOS (VERIFICACIÓN ENDURECIDA)
    -- ------------------------------------------------------------------------
    ...
    -- E. Comprobación explícita de los 13 pares exactos: UUID <-> rfef_club_id
    SELECT COUNT(*) INTO v_verificados_13
    FROM (
        VALUES
            ('9a76a56a-3195-4b4b-9ff0-054731d6dda8'::uuid, 23289700, 'SD LEIOA'),
            ('cb701fe7-be63-4540-85b6-9296a165a424'::uuid, 33836521, 'CULTURAL LEONESA'),
            ('5ed38a15-75db-4752-8706-634b92d898b5'::uuid, 205484,   'DEPORTIVO ALAVES'),
            ('c4e8af5c-bc98-4c5d-9751-c6c54b502422'::uuid, 33836523, 'ARRATIA'),
            ('de9fd8d8-04fa-4c19-ac29-c2e41352b375'::uuid, 205567,   'SANTUTXU FC'),
            ('a764b115-09dd-4fbd-be05-c7d8f36a54bc'::uuid, 900361152, 'DANOK BAT'),
            ('aa8de564-c6cb-4689-9416-dbffb9c5bf56'::uuid, 33836522, 'EF MAREO'),
            ('e686b482-b4c2-4ed0-acf2-3bded5585c4c'::uuid, 205459,   'REAL VALLADOLID'),
            ('b66776ce-a334-4d59-998d-f8726fea9dce'::uuid, 23289793, 'BETOÑO'),
            ('9b131d39-b143-488b-851d-a55c140c1396'::uuid, 205540,   'SD EIBAR'),
            ('8af137d7-ed20-4a9e-aebc-3a515b093d26'::uuid, 207449,   'UNIONISTAS SALAMANCA'),
            ('7534c39f-8b17-47e2-8dd8-b3f238ed9ace'::uuid, 205603,   'ANTIGUOKO KE'),
            ('a823e75c-2246-4b19-9dd1-89ab199ebe38'::uuid, 205597,   'REAL SOCIEDAD')
    ) AS expected(id, rfef_club_id, nombre)
    JOIN public.clubs c 
      ON c.id = expected.id 
     AND c.rfef_club_id = expected.rfef_club_id 
     AND c.nombre = expected.nombre
     AND c.tipo = 'LIGA';

    IF v_verificados_13 <> 13 THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Uno o más de los 13 rivales no tienen exactamente el par UUID <-> rfef_club_id esperado (verificados: %/13)', v_verificados_13;
    END IF;

    -- F. Comprobar SD INDAUTXU localizándolo estrictamente por rfef_club_id = 33836524
    SELECT id, tipo, nombre INTO v_indautxu_id, v_indautxu_post_tipo, v_indautxu_post_nombre
    FROM public.clubs
    WHERE rfef_club_id = 33836524;

    IF v_indautxu_id IS NULL THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] No se encontró el registro de SD Indautxu con rfef_club_id = 33836524';
    END IF;

    IF v_indautxu_post_tipo <> 'PROPIO' OR v_indautxu_post_nombre <> 'SD INDAUTXU' THEN
        RAISE EXCEPTION '[POSTCHECK FALLIDO] Registro de Indautxu con datos inesperados (tipo: %, nombre: %)', v_indautxu_post_tipo, v_indautxu_post_nombre;
    END IF;

    -- G. Athletic y Logroñés intactos
    ...
    RAISE NOTICE '============================================================';
    RAISE NOTICE '   RFEF · J1-C.1 · TRANSACCIÓN VALIDADA EXITOSAMENTE (PASS) ';
    RAISE NOTICE '   - 16 Clubes configurados (15 LIGA + 1 PROPIO)            ';
    RAISE NOTICE '   - 16 rfef_club_id únicos asignados                       ';
    RAISE NOTICE '   - SD Indautxu (33836524 / PROPIO) UUID generado: %       ', v_indautxu_id;
    RAISE NOTICE '   - 13 pares UUID <-> rfef_club_id verificados (13/13)     ';
    RAISE NOTICE '   - Athletic (205514) y Logroñés (205744) intactos         ';
    RAISE NOTICE '============================================================';
END $$;
